...
/*... loop nas celulas*/
  aux2 = maxViz + 1;
#pragma omp parallel  for default(none) num_threads(nThreads) if(ompVar.fCell)\
   private(nel,i,j,aux1,lId,lMat,lib,lVolume,lGeomType,lu0\
          ,lFaceR,lA,lB,lProp,lGradU0\
   ...
   shared(aux2,ndm,ndf,numel,maxViz,calRcell,rCell,nFace,mat\
         ...
         ,density,cDiff,prop,gradU0,vel,fModKsi\
 ...
 * gradRho   -> gradiente da massa especifica                        *
...
/*... variavel local */
  ...
  short  lGeomType[MAX_NUM_FACE+1],
         lFaceVelR[MAX_NUM_FACE+1], lFacePresR[MAX_NUM_FACE+1];
  INT    lId[(MAX_NUM_FACE+1)*MAX_NDF], lViz[MAX_NUM_FACE];
  INT    idFace, cellOwner, ch;
  DOUBLE lKsi[MAX_NUM_FACE*MAX_NDM]    , lmKsi[MAX_NUM_FACE],
...
#pragma omp parallel  for default(none) if(ompVar.fCell)\
  ...
  shared(aux2,ndm,ndf,numel,maxViz,calRcell,rCell,nFace,mat,ntn\
         ...
         ,density,gradPres,vel,gCc,gradVel,fModKsi,gradRho\
         ,fArea,gDcca,fModvSkew,fKsi,fEta,fNormal,fXm,gXmCc,fvSkew\
...
/*...*/
    aux1    = nFace[nel];
/*... elementos sem equacoes*/
    if(MAT2D(nel, aux1, faceVelR, aux2) > 0)
      continue;
/*...................................................................*/
...
#pragma omp parallel  for default(none) num_threads(nThreads) if(ompVar.fCell)\
    private(nel,i,j,k,aux1,lId,lMat,lib,lVolume,lGeomType,lu0\
          ,lFaceR,lA,lB,lDensity,lGradU0,lDfield\
          ,lVel,lCc,lmKsi,lfArea,lDcca,lmvSkew,lKsi,lEta,lViscosity\
          ,lNormal,lXm,lXmcc,lvSkew,vizNel,lViz,lRcell,lPres,lGradPres\
          ,lWallPar,idFace,cellOwner,ch\
          ,lDiff,lWk)\
     shared(aux2,ndm,ndf,numel,maxViz,calRcell,rCell,nFace,mat\
         ,calType,gVolume,geomType,faceR\
         ,u0,gCc,loads,ldVel,dField\
...
/*... variavel local */
  ...
  short  lib;
  short  lFaceVelR[MAX_NUM_FACE+1],lFacePresR[MAX_NUM_FACE+1];
  DOUBLE lDensity[(MAX_NUM_FACE+1)*3];
  DOUBLE lA[(MAX_NUM_FACE+1)*MAX_NDF],lB[MAX_NDF];
  ...
  DOUBLE lVel[(MAX_NUM_FACE+1)*MAX_NDM];
  DOUBLE lDfield[(MAX_NUM_FACE+1)*MAX_NDM];
  DOUBLE lRcell,lWallPar[NWALLPAR],lMolar[3];
  INT    lId[(MAX_NUM_FACE+1)*MAX_NDF],lViz[MAX_NUM_FACE];
  INT idFace, cellOwner, ch;
  short  aux1,aux2,lMat;
...
/*...*/
    aux1    = nFace[nel];
/*... elementos sem equacoes*/
    if(MAT2D(nel, aux1, facePresR, aux2) > 0)
      continue;
/*...................................................................*/
...
/*... loop na celula central*/  
    ...
    lFacePresR[aux1]= MAT2D(nel,aux1,facePresR ,aux2);
/*...*/
    MAT2D(aux1,0,lDensity, 3) = density.t00[nel]; 
    MAT2D(aux1,1,lDensity, 3) = density.t0[nel];
    MAT2D(aux1,2,lDensity, 3) = density.t[nel];
/*...................................................................*/ 

/*...*/
    lMolar[0] = mMolar.t00[nel];
    lMolar[1] = mMolar.t0[nel];
    lMolar[2] = mMolar.t[nel];
/*...................................................................*/ 
...
/*... loop na celulas vizinhas*/    
    for(i=0;i<aux1;i++)
    {
      ...
      if( vizNel != -2)
      {
        ...
        lId[i]        = id[vizNel] - 1;
/*...*/
        MAT2D(i,0,lDensity, 3) = density.t00[vizNel]; 
        MAT2D(i,1,lDensity, 3) = density.t0[vizNel];
        MAT2D(i,2,lDensity, 3) = density.t[vizNel];
/*...................................................................*/
 ...
 * Data de modificaco : 06/10/2019                                   * 
 *-------------------------------------------------------------------* 
 * WALLFLUID : identifica as paredes impermeveis estaticas e         *   
